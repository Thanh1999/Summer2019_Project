...
public class FilterDispatcher implements Filter {
    ...
    private final String LOCK_PAGE = "login.jsp";
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        //Lấy session để xem có tồn tại hay ko
        HttpSession session = req.getSession(false);
        try {
            int lastIndex = uri.lastIndexOf("/");
            //Lấy action truyền đến, action viết hoa + Servlet để forward đến servlet xử lý 
            String resource = uri.substring(lastIndex + 1);
            if (resource.length() > 0) {
                url = resource.substring(0, 1).toUpperCase()
                        + resource.substring(1)
                        + "Servlet";
                //Nếu là trang jsp hay html thì khỏi viết hoa hay + Servlet
                if (resource.lastIndexOf(".html") > 0 || resource.lastIndexOf(".jsp") > 0) {
                    url = resource;

                }
                //Nếu người dùng chưa đăng nhập thì ko dc vào trang nào khác ngoài LOGIN_PAGE
                if (session == null) {
                    if (!url.equals("LoginServlet")) {
                        url = LOGIN_PAGE;
                    }
                } else {
                    //Kiểm tra xem account có bị khoá ko nếu có thì vào LOCK_PAGE bất kể url là gì
                    if (session.getAttribute("UNLOCK") != null) {
                        //Nếu thời gian hiện tại >= thời gian khoá thì huỷ session ve LOGIN_PAGE
                        Calendar c = Calendar.getInstance();                        
                        if (c.getTimeInMillis() >= (long) session.getAttribute("UNLOCK")) {
                            session.invalidate();
                        }
                        url = LOCK_PAGE;
                    } 
                    //Nếu account ko bị khoá mà chưa có tên người dùng cũng về lại LOGIN_PAGE
                    else if (session.getAttribute("USER") == null) {
                        //Nếu người dùng làm bất cứ hành động nào khác login thì sẽ lại LOGIN_PAGE
                        if (!url.equals("LoginServlet") && !url.equals("invalid.html")) {
                            url = LOGIN_PAGE;
                        }
                    }
                }
